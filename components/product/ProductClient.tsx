...
export default function ProductClient({ product }: { product: Product }) {
  ...
  const [selectedDesign, setSelectedDesign] = useState<string | undefined>(product.designs?.[0])
  ...
  const handleAddToCart = () => {
    addToCart({
      productId: (product._id || product.id || product.slug) as string,
      quantity,
                      ...
                      {product.colors.map((color) => (
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                            selectedColor === color ? "bg-primary text-primary-foreground ring-2 ring-primary" : "bg-muted text-foreground hover:bg-muted/80"
                          }`}
                        >
                          {color}
                        </button>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {product.designs.map((design) => (
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                            selectedDesign === design ? "bg-primary text-primary-foreground ring-2 ring-primary" : "bg-muted text-foreground hover:bg-muted/80"
                          }`}
                        >
                          {design}
                        </button>
